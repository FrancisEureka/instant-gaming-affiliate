...
export default function Sidebar({
  ...
  const content = (
    <div className="flex flex-col h-full text-slate-200">
      {/* Brand & Profile Header com Logo Oficial */}
      <div className="flex flex-col items-center pb-5 border-b border-slate-800/80">
        <div className="relative group mb-3">
          <div className="absolute -inset-1.5 rounded-full bg-gradient-to-r from-emerald-500 via-cyan-500 to-indigo-500 opacity-80 blur-md group-hover:opacity-100 transition duration-300"></div>
          <div className="relative w-20 h-20 rounded-full bg-slate-950 border-2 border-emerald-400 overflow-hidden flex items-center justify-center shadow-2xl">
            <img
              src="/logo.webp"
        ...
        <h2 className="text-xl font-black tracking-tight text-white flex items-center gap-1.5">
          Ofertas do Eureka
          <span className="inline-block w-2.5 h-2.5 rounded-full bg-emerald-400 shadow-[0_0_8px_#22c55e] animate-pulse"></span>
        </h2>
        <p className="text-xs text-slate-400 mt-0.5 text-center font-medium">
          As Melhores Promoções & Jogos Grátis
        </p>

        <div className="flex items-center gap-2 mt-3 px-3 py-1 rounded-full bg-slate-900/90 border border-slate-800 text-[11px] text-slate-300">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
          <span>{totalDeals} Ofertas • {totalFree} Grátis</span>
        </div>
      </div>

      {/* Social Links (Todas as Mídias) */}
      <nav aria-label="Mídias Oficiais" className="py-4 border-b border-slate-800/80">
        ...
        <div className="grid grid-cols-1 gap-1">
          {SOCIAL_LINKS.map((link) => (
            <a
              ...
              className={`flex items-center gap-2.5 px-3 py-1.5 text-xs font-medium rounded-lg text-slate-300 hover:text-white hover:bg-slate-800/60 transition-all duration-200 border border-transparent ${link.color}`}
            >
              <span className="p-1 rounded-md bg-slate-900/80">{link.icon}</span>
      ...
      <div className="pt-3 border-t border-slate-800/80 text-[10px] text-slate-400 text-center">
        ...
        <p className="mt-0.5 text-slate-400">© 2026 Ofertas do Eureka</p>
      </div>
    </div>
  )
